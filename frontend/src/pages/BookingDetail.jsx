...
// האם ה-checkout עדיין בעתיד (ההזמנה טרם הסתיימה)?
...
function buildMessageBody(templateId, lang, booking, ttlockCode, paymentLink, checkinTime, checkoutTime) {
  ...
  return text
    ...
    .replace(/\{שעת_כניסה\}/g,     checkinTime  || DEFAULT_CHECKIN_TIME)
    .replace(/\{שעת_יציאה\}/g,     checkoutTime || DEFAULT_CHECKOUT_TIME)
...
export default function BookingDetail({ bookingId, navigate }) {
  ...
  // ── שעות: להזמנות עתידיות (טרם הסתיימו) — ברירת מחדל אחידה 14:00/12:00, ──
  //    אלא אם נערך ידנית בסשן הנוכחי. להזמנות עבר — הערך השמור מ-DB.
  const upcoming = isUpcoming(booking);
  const checkinTime = localCheckinTime !== null
    ? localCheckinTime
    : (upcoming ? DEFAULT_CHECKIN_TIME : (booking.checkin_time || DEFAULT_CHECKIN_TIME));
  const checkoutTime = localCheckoutTime !== null
    ? localCheckoutTime
    : (upcoming ? DEFAULT_CHECKOUT_TIME : (booking.checkout_time || DEFAULT_CHECKOUT_TIME));
